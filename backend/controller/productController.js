...
const addproperty = async (req, res) => {
    try {
        ...
        // Safely extract up to 8 images
        const images = [
            req.files?.image1?.[0], req.files?.image2?.[0], req.files?.image3?.[0], req.files?.image4?.[0],
            req.files?.image5?.[0], req.files?.image6?.[0], req.files?.image7?.[0], req.files?.image8?.[0]
        ].filter((item) => item !== undefined);

        // 🚨 Validation Rules 🚨
        if (images.length === 0) {
            return res.status(400).json({ message: "At least 1 image is required.", success: false });
        }
        if (type !== 'Plot') {
            if (!beds || !baths) {
                return res.status(400).json({ message: "Bedrooms and Bathrooms are required unless the property is a Plot.", success: false });
            }
        }
