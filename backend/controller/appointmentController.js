    ...
    if (recipientEmail) {
      try {
        const mailOptions = {
          ...
          to: recipientEmail,
          subject: `Viewing Appointment ${status.charAt(0).toUpperCase() + status.slice(1)} - PlotFarm`,
    ...
    if (recipientEmail) {
      try {
        const mailOptions = {
          from: process.env.EMAIL,
          to: recipientEmail,
          subject: 'Viewing Scheduled - PlotFarm',
    ...
    if (recipientEmail) {
      try {
        const mailOptions = {
          from: process.env.EMAIL,
          to: recipientEmail,
          subject: 'Appointment Cancelled - PlotFarm',
    ...
    if (recipientEmail) {
      try {
        const mailOptions = {
          from: process.env.EMAIL,
          to: recipientEmail,
          subject: 'Meeting Link Updated - PlotFarm',
