...
apiClient.interceptors.request.use(
  (config) => {
    const token = localStorage.getItem('plotfarm_token');
    if (token) {
...
apiClient.interceptors.response.use(
  ...
  (error) => {
    if (error.response?.status === 401) {
      localStorage.removeItem('plotfarm_token');
...
export const aiAPI = {
  search: (data: {
    ...
    const githubKey    = localStorage.getItem('plotfarm_github_key');
    const firecrawlKey = localStorage.getItem('plotfarm_firecrawl_key');
  ...
  validateKeys: (keys?: { githubKey?: string; firecrawlKey?: string }) => {
    const githubKey = (keys?.githubKey ?? localStorage.getItem('plotfarm_github_key') ?? '').trim();
    const firecrawlKey = (keys?.firecrawlKey ?? localStorage.getItem('plotfarm_firecrawl_key') ?? '').trim();
...
export const apiKeyStorage = {
  getGithubKey:    ()    => localStorage.getItem('plotfarm_github_key') || '',
  getFirecrawlKey: ()    => localStorage.getItem('plotfarm_firecrawl_key') || '',
  setGithubKey:    (key: string) => localStorage.setItem('plotfarm_github_key', key),
  setFirecrawlKey: (key: string) => localStorage.setItem('plotfarm_firecrawl_key', key),
  hasKeys: () => !!(localStorage.getItem('plotfarm_github_key') && localStorage.getItem('plotfarm_firecrawl_key')),
  clear: () => {
    localStorage.removeItem('plotfarm_github_key');
    localStorage.removeItem('plotfarm_firecrawl_key');
  },
};
